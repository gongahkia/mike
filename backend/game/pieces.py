...
class Piece:
    ...
    def get_moves(self, row: int, col: int, board, check_validity: bool = True) -> List[Tuple[int, int]]:
        ...
        # Only filter for check validity if requested (to avoid recursion)
        if check_validity:
            valid_moves = []
    ...
    def _is_valid_move(self, from_row: int, from_col: int, to_row: int, to_col: int, board) -> bool:
        ...
        # Check if own king is in check (use check_validity=False to avoid recursion)
        in_check = board.is_in_check(self.player, check_validity=False)
