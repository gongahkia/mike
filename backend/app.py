...
def get_ai_move(game_id):
    ...
    # Convert ai_move to JSON-serializable format
    serializable_ai_move = {}
    for key, value in ai_move.items():
        if hasattr(value, 'value'):  # Handle enum values
            serializable_ai_move[key] = value.value
        elif hasattr(value, 'to_dict'):  # Handle Piece objects
            serializable_ai_move[key] = value.to_dict()
        else:
            serializable_ai_move[key] = value
    
    response = {
        'success': True,
        'ai_move': serializable_ai_move,
        'game_state': game.get_game_state(),
        'game_over': result.get('game_over', False)
    }
